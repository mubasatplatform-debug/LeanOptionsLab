...
### 1.1 🪤 ما يشحنه مصدر LEAN لـSPY لا يُحتسب — لا تبذره

ثلاثة من البنود الستة **موجودة بالاسم** في `.tools/lean-engine/Data/` لكنها لا تغطّي الفترة (مقيس 2026-09-02):

| الملف                                             | ما يغطّيه فعلاً             | داخل نافذة 2021‑2025 |
| ------------------------------------------------- | --------------------------- | -------------------- |
| `equity/usa/daily/spy.zip`                        | 1998‑01‑02 → **2021‑03‑31** | 61 صفاً فقط          |
| `equity/usa/hour/spy.zip`                         | 1998‑01‑02 → **2021‑03‑31** | 427 صفاً فقط         |
| `alternative/interest-rate/usa/interest-rate.csv` | 2003‑01 → **2023‑07‑27**    | جزئي                 |

تقرير الفجوة يقرأ **الطلبات الفاشلة**، فملف موجود ولو ناقصاً يجعل الطلب «ناجحاً» ويختفي من القائمة. بذر هذه الملفات يُخفي ثلاثة بنود دون أن يسدّها، ويجعل التجربة تبدو أقرب مما هي. اتركها في مصدر LEAN، واحصل على النسخ الكاملة مع بقية البيانات.
...
### 2.2 أمر واحد يفعل كل ما سبق

الخطوات أعلاه مجمّعة في سكربت **خارج المستودع** (بوابة الجودة تمنع اسم CLI في السكربتات):

```powershell
C:\Users\PC\_home-tidy\scripts\lean-options-lab-acquire-spy-data.ps1
```

يُنشئ جذر CLI في `C:\Users\PC\quantconnect-cli` (يرفض أي مسار داخل المستودع)، ويشغّل `login` **لتكتب أنت** الاعتماد، ثم `init`، ثم معالج التنزيل التفاعلي، ثم ينسخ **مسارات الفجوة فقط** إلى `data/`، ثم يعيد Backtest وتقرير الفجوة. جرّبه أولاً بلا أثر:

```powershell
C:\Users\PC\_home-tidy\scripts\lean-options-lab-acquire-spy-data.ps1 -DryRun
```

معاملات الوضع غير التفاعلي، **مقروءة من `lean data download --help` في الإصدار 1.0.229**:

```text
--data-provider-historical QuantConnect
--dataset <الاسم>   --security-type Equity|Option   --market USA   --ticker SPY
--resolution Minute|Hour|Daily   --data-type Trade|Quote|Universe|OpenInterest
--start 20201231   --end 20251231
```

- ⚠️ `-y` / `--yes` «يؤكّد مطالبات الدفع تلقائياً» — السكربت **لا يمرّره أبداً**. نصّ المساعدة يقول إن الوضع غير التفاعلي «يتوقف فقط عند اتفاقية البيانات»، وهو غامض بشأن الدفع؛ لذلك الوضع الافتراضي في السكربت هو **المعالج التفاعلي**، و`-NonInteractive` اختياري بتحذير.
- أسماء `--dataset` (`US Equities`، `US Equity Options`، `US Equity Option Universe`) مقروءة من صفحات مجموعات البيانات **لا من CLI**، وقد تُرفض؛ عندها يعود السكربت إلى المعالج.
- `lean whoami` يكشف حالة الدخول، والسكربت يستخدمه ليتخطّى `login` إن كان الجهاز مصادَقاً أصلاً.
